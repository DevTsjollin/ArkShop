...
namespace ArkShop::Store
{
	 ...
	 */
	void sendDiscordMessage(std::string message) {
		nlohmann::json j;
		FString Fservername;
		ArkApi::GetApiUtils().GetShooterGameMode()->GetMapName(&Fservername);
		std::string servername = Fservername.ToString();
		try {
			j["content"] = message;
			j["username"] = servername;
			const std::string jsonMessage = j.dump();

			static_cast<AShooterGameState*>(ArkApi::GetApiUtils().GetWorld()->GameStateField())->HTTPPostRequest(FString(config.value("WebhookLogger", "none")), FString(ArkApi::Tools::Utf8Decode(jsonMessage).c_str()));
		}
	...
	bool Buy(AShooterPlayerController* player_controller, const FString& item_id, int amount)
	{
		...
		if (DBHelper::IsPlayerExists(steam_id))
		{
			...
			if (success)
			{
				...
				sendDiscordMessage(ArkApi::Tools::Utf8Encode(log));
			}
		}
